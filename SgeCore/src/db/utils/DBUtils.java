...
public class DBUtils {
	 ...
	 * @return
	 */
	public static long insertAll(final ArrayList<? extends BaseDbBean> dbBeanList, final String feldName) {
	...
	private static long dbExcute(ArrayList<? extends BaseDbBean> dbBeanList, DBExcuteInterf dbExcuteInterf) {
		long rows = 0;
	...
	public static long insertAll(final ArrayList<? extends BaseDbBean> dbBeanList) {
	 ...
	 * @return 影响了多少行数据
	 */
	public static long updateAll(final ArrayList<? extends BaseDbBean> dbBeanList, final String feldName,final String[]keys) {
		...
		return dbExcute(dbBeanList, new DBExcuteInterf() {
			...
			public long[] excute(SQLiteDatabase db, String strTableName, long rows) {
				...
				for (int i = 0; i < dbBeanList.size(); i++) {
					...
					rows += db.update(strTableName, ((BaseDbBean) dbBeanList.get(i)).contentValues(keys), column + "=?",
							new String[] { value });
				}
